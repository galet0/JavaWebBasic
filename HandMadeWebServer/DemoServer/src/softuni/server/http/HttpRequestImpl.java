...
public class HttpRequestImpl implements HttpRequest {
    ...
    private void parseRequest(String requestString, Map<String, HttpSession> sessionMap) throws BadRequestException, UnsupportedEncodingException {
        ...
        String reqMethod = requestLine[0].toLowerCase();
        this.requestMethod = this.parseRequestMethod(reqMethod);
        ...
        if(this.requestMethod.equals(HttpRequestMethod.POST)){
    ...
    private void parseCookie() {
        ...
        String[] cookies = cookieString.trim().split(";\\s*");

        for (String s : cookies) {
            String[] cookieArgs = s.split("=");
    ...
    private void parseQuery(String query, Map<String, String> queryParameters) throws UnsupportedEncodingException {
        if(!query.contains("=")){
            return;
        }
        ...
        for (String queryPair : queryPairs) {
            ...
            queryParameters.put(URLDecoder.decode(queryArgs[0], "UTF-8"),
    ...
    private void parseHeaders(String[] requestLines) throws BadRequestException {
        ...
        for (int i = 1; i < endIndex; i++) {
            String[] headersArgs = requestLines[i].split(": ");
            this.headers.put(headersArgs[0], headersArgs[1]);
        }
